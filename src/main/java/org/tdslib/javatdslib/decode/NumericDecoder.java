...
public class NumericDecoder implements ResultDecoder {
  ...
  public boolean canDecode(TdsType tdsType) {
    switch (tdsType) {
      case INT1: case INT2: case INT4: case INT8: case INTN:
      case FLT4: case FLT8: case FLTN:
      case BIT: case BITN:
      case NUMERIC: case DECIMAL: case NUMERICN: case DECIMALN:
      case MONEY: case MONEYN: case SMALLMONEY:
        return true;
      default:
        return false;
    }
  ...
  public <T> T decode(byte[] data, TdsType tdsType, Class<T> targetType, int scale, Charset varcharCharset) {
    switch (tdsType) {
      ...
      case INT2: return convertSimple(ByteBuffer.wrap(data).order(ByteOrder.LITTLE_ENDIAN).getShort(), targetType);
      case INT4: return convertSimple(ByteBuffer.wrap(data).order(ByteOrder.LITTLE_ENDIAN).getInt(), targetType);
      case INT8: return convertSimple(ByteBuffer.wrap(data).order(ByteOrder.LITTLE_ENDIAN).getLong(), targetType);
      case INTN:
        if (data.length == 1) return convertSimple(data[0] & 0xFF, targetType); // FIX: Unsigned mask
        if (data.length == 2) return convertSimple(ByteBuffer.wrap(data).order(ByteOrder.LITTLE_ENDIAN).getShort(), targetType);
        if (data.length == 4) return convertSimple(ByteBuffer.wrap(data).order(ByteOrder.LITTLE_ENDIAN).getInt(), targetType);
        if (data.length == 8) return convertSimple(ByteBuffer.wrap(data).order(ByteOrder.LITTLE_ENDIAN).getLong(), targetType);
        throw new IllegalStateException("Unexpected INTN length");

      case FLT4: return targetType.cast(ByteBuffer.wrap(data).order(ByteOrder.LITTLE_ENDIAN).getFloat());
      ...
      case FLT8:
        ...
        if (targetType == Float.class) return targetType.cast((float) dVal);
        return targetType.cast(dVal);

      case BIT:
      case BITN:
        boolean bVal = data[0] != 0;
        if (targetType == Boolean.class) return targetType.cast(bVal);
        return convertSimple(bVal ? 1 : 0, targetType);
      ...
      case SMALLMONEY:
        ...
        throw new UnsupportedOperationException("Money conversion to " + targetType.getName() + " not supported");
