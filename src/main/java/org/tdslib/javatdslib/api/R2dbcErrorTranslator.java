...
public class R2dbcErrorTranslator {
  ...
  public static R2dbcException translateException(TdsServerErrorException tdsError) {
    int errorCode = tdsError.getErrorNumber();
    String msg = tdsError.getMessage();
    String sqlState = mapToSqlState(errorCode);

    return switch (errorCode) {
      case 208, 2812 -> new R2dbcBadGrammarException(msg, sqlState, errorCode);
      case 547, 2601, 2627 -> new R2dbcDataIntegrityViolationException(msg, sqlState, errorCode);
      case 229, 230 -> new R2dbcPermissionDeniedException(msg, sqlState, errorCode);
      case 1205 -> new R2dbcTransientResourceException(msg, sqlState, errorCode);
      default -> {
        if (tdsError.getSeverity() >= 20) {
          yield new R2dbcNonTransientResourceException(msg, sqlState, errorCode);
        }
        yield new R2dbcException(msg, sqlState, errorCode) {};
      }
    };
  }

  private static String mapToSqlState(int errorCode) {
