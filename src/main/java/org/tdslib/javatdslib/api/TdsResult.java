...
public class TdsResult implements Result {
  ...
  /**
   * Constructs a new TdsResult.
   *
   * @param source The publisher of result segments.
   */
  ...
  private static class SegmentFlatMapPublisher<T> implements Publisher<T> {
    ...
    SegmentFlatMapPublisher(
        Publisher<Result.Segment> source,
        Function<Result.Segment, ? extends Publisher<? extends T>> mapper) {
      this.source = source;
      this.mapper = mapper;
    }
