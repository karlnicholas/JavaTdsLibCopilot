...
public class PreLoginPhase {
      ...
      while (combined.hasRemaining()) {
        ...
        switch (option) {
          ...
          case 0x04: // PACKETSIZE
            ...
            try {
              response.setNegotiatedPacketSize(Integer.parseInt(new String(psBytes, StandardCharsets.US_ASCII)));
  ...
  private ByteBuffer combinePayloads(List<TdsMessage> packets) {
    ...
    for (TdsMessage m : packets) combined.put(m.getPayload().duplicate());
    combined.flip();
    return combined;
  }
}