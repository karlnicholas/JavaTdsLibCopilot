...
/**
 * Holds constants and helper methods for TDS data types to avoid hard-coding in parsers.
 * This class can be expanded as more types are supported.
 *
 * Note: This focuses on common types for result sets. Nullable types (*N) use BYTE length prefix,
 * fixed non-nullable use direct data, variables use USHORT.
 * Corrected based on TDS specification: INT (4 bytes) is 0x38, TINYINT (1 byte) is 0x30, etc.
 */
public class TdsDataTypes {
    ...
    public static final byte SMALLDATETIMETYPE = 0x3A; // Smalldatetime (4 bytes, fixed) - SYBDATETIME4
    ...
    // Variable types like VARCHAR (0xA7) use USHORT len.
    ...
    public static int getFixedLength(byte type) {
        switch (type) {
            case TINYINTTYPE: return 1;
            case SMALLINTTYPE: return 2;
            case INTTYPE: return 4;
            case REALTYPE: return 4;
            case FLOATTYPE: return 8;
            case SMALLDATETIMETYPE: return 4;
            case BIGINTTYPE: return 8;
            default: return 0;
        }
    }
