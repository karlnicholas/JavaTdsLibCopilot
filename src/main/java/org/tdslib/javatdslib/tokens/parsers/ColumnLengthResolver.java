...
public class ColumnLengthResolver {
   ...
   * * @return The length in bytes to read, or -1 if the value is SQL NULL.
   * @throws BufferUnderflowException if the buffer does not have enough bytes to read the length header.
   */
    ...
    switch (type.strategy) {
      case FIXED:
        if (type == TdsType.DATE) {
          if (payload.remaining() < 1) throw new BufferUnderflowException();
      ...
      case USHORTLEN:
        ...
        if (payload.remaining() < 2) throw new BufferUnderflowException();
